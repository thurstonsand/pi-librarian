...
The constrained environment installs this package with `pi install git:github.com/thurstonsand/pi-librarian@downgrade`, and `pi update` hard-resets that clone to the branch tip. The branch must therefore carry the release, or that install stays on the previous version.
